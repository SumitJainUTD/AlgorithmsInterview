...
public class KthSmallestElement {
	...
	public int kSmall(int [] arrA, int start, int end,int k){
		...
		int pivot = start;
		while(left<=right){
			while(left<=right && arrA[left]<=arrA[pivot])left++;
			while(left<=right && arrA[right]>=arrA[pivot])right--;			
			if(left<right){
		...
		if(pivot==k)return arrA[pivot];
		else if(pivot<k)return kSmall(arrA, pivot+1, end, k);
		else return kSmall(arrA, start, pivot-1, k);
	}
