...
public class NbyKTimes {
	...
	public void findElement(int [] arrA, int k){
		...
		// now check if any of the elements in the Elements array appears more than n/k times
		for(int i=0;i<emts.length;i++){
			...
			for(int j=0;j<arrA.length;j++){
				...
				if(cnt>(arrA.length/k)){
					System.out.println(emts[i].element + " appears more than n/"+ k+ " times, Actual count is " + cnt);
					return;
				}
			}
		}
		System.out.println("NONE appears more than n/"+ k+ " times");
	...
	public static void main(String args[]){
		int [] arrA = {1,2,3,4,3,5,3,6,3,3,8};
		NbyKTimes n = new NbyKTimes();
		n.findElement(arrA, 3);
	}
}
class Elements{
